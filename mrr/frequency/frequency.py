...
def _calc_frequencies_(extrema, weights=None):
    """
    Calculate the frequency from a number of extrema.
...
def average_frequencies(t, data, error, N=1500, s=None, weights=None,
    ...
    for nset in range(noofsets):
        ...
        extrema = _find_extrema(t_, d_, w=w_, s=s_, t0=t0, t1=t1,
                                show_plot=show_plot, ploterr=e_)
        n = extrema.size
    ...
    if print_infos:
        for i in range(noofsets):
            print "{:.3e} {:.3e}".format(samplefrequencies[i], dfreqs[i])

    if full_return:
        return f, df, np.vstack([samplefrequencies, dfreqs])
    else:
        return f, df
